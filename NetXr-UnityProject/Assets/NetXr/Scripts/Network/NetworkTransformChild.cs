//======= Copyright (c) Unknown. ===============
//
// Source: Unity Network Code
// Modified Version of NetworkTransform
//
//=============================================================================
...
namespace NetXr {
    public class NetworkTransformChild : NetworkBehaviour {
        const short CustomNetworkTransformChildMsg = 1002;
        ...
        void OnValidate () {
            ...
            if (m_Target != null) {
                ...
                if (parent == null) {
                    if (LogFilter.logError) { Debug.LogError ("CustomNetworkTransformChild.OnValidate: target cannot be the root transform."); }
                    m_Target = null;
                    return;
                }
                ...
                if (m_Root == null) {
                    if (LogFilter.logError) { Debug.LogError ("CustomNetworkTransformChild.OnValidate: root must have NetworkTransform"); }
                    m_Target = null;
                    return;
                }
            }
            ...
            if (m_ChildIndex == UInt32.MaxValue) {
                if (LogFilter.logError) { Debug.LogError ("CustomNetworkTransformChild.OnValidate: component must be a child in the same hierarchy"); }
                m_Target = null;
            }
        ...
        void Start () {
            ...
            // from https://docs.unity3d.com/ScriptReference/Networking.NetworkClient.RegisterHandler.html
            //      NetworkClient.RegisterHandler(MsgType.Connect, OnConnected);
            // in UnityEngine.Networking.NetworkServer
            //      m_SimpleServerSimple.RegisterHandlerSafe(MsgType.LocalChildTransform, NetworkTransformChild.HandleChildTransform);
            NetworkClient.allClients[0].RegisterHandler (CustomNetworkTransformChildMsg, HandleChildTransform); // CustomNetworkTransformChild.HandleChildTransform
            NetworkServer.RegisterHandler (CustomNetworkTransformChildMsg, HandleChildTransform);
        }
        ...
        void SendTransform () {
            ...
            m_LocalTransformWriter.StartMessage (CustomNetworkTransformChildMsg);
            ...
            UnityEditor.NetworkDetailStats.IncrementStat (
                ...
                MsgType.CustomLocalChildTransform, "16:CustomLocalChildTransform", 1);
            #endif
        ...
        static internal void HandleChildTransform (NetworkMessage netMsg) {
            ...
            if (foundObj == null) {
                if (LogFilter.logError) { Debug.LogError ("CustomNetworkTransformChild.HandleChildTransform: no gameObject"); }
                return;
            }
            var children = foundObj.GetComponents<NetworkTransformChild> ();
            if (children == null || children.Length == 0) {
                if (LogFilter.logError) { Debug.LogError ("CustomNetworkTransformChild.HandleChildTransform: no children " + children + " " + foundObj.name); }
                return;
            }
            if (childIndex >= children.Length) {
                if (LogFilter.logError) { Debug.LogError ("CustomNetworkTransformChild.HandleChildTransform: childIndex invalid"); }
                return;
            }

            NetworkTransformChild foundSync = children[childIndex];
            if (foundSync == null) {
                if (LogFilter.logError) { Debug.LogError ("CustomNetworkTransformChild.HandleChildTransform: null target"); }
                return;
            }
            if (!foundSync.localPlayerAuthority) {
                if (LogFilter.logError) { Debug.LogError ("CustomNetworkTransformChild.HandleChildTransform: no localPlayerAuthority"); }
                return;
            }

            if (!netMsg.conn.clientOwnedObjects.Contains (netId)) {
                if (LogFilter.logWarn) { Debug.LogWarning ("CustomNetworkTransformChild.HandleChildTransform: netId:" + netId + " is not for a valid player"); }
                return;
            }
